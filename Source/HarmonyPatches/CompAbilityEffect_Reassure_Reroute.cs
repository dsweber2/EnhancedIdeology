...
internal static class CompAbilityEffect_Reassure_Apply
{
    private static bool Prefix(CompAbilityEffect_Reassure __instance, LocalTargetInfo target)
    {
        ...
        if (issues.Count == 0)
        {
            Messages.Message("EnhancedIdeology.Reassure.NoHeterodoxy".Translate(),
        ...
        {
            Messages.Message("EnhancedIdeology.Reassure.FailMessage".Translate(
...
internal static class CompAbilityEffect_Reassure_Tooltip
{
    private static bool Prefix(CompAbilityEffect __instance, LocalTargetInfo target, ref string? __result)
    {
        ...
        sb.AppendLine("EnhancedIdeology.Reassure.CertaintyGain".Translate(AbilityReassure.ReassureCertaintyGain.ToStringPercent()));

        if (issues.Count == 0)
        {
            sb.AppendLine("EnhancedIdeology.Reassure.NoHeterodoxy".Translate());
        }
        else
        {
            if (guide == recipient)
            {
                sb.AppendLine("EnhancedIdeology.Reassure.SelfTarget".Translate());
            }
            else
            {
                sb.AppendLine("EnhancedIdeology.Reassure.SuccessChance".Translate(
                    InteractionWorker_IdeologicalDebatePrecept.WinChance(guide, recipient).ToStringPercent()));
            }

            sb.AppendLine("EnhancedIdeology.Reassure.TargetBeliefs".Translate(AbilityReassure.MaxBundleIssues));
