...
internal sealed class CompReligiousBook : ThingComp
{
    ...
    private static void RecordEventWithOptionalDoer(Pawn? instigator)
    {
        if (instigator != null)
        {
            Find.HistoryEventsManager.RecordEvent(new HistoryEvent(EnhancedIdeologyDefOf.EB_BookDestroyed, instigator.Named(HistoryEventArgsNames.Doer)), canApplySelfTookThoughts: false);
        }
        else
        {
            Find.HistoryEventsManager.RecordEvent(new HistoryEvent(EnhancedIdeologyDefOf.EB_BookDestroyed));
        }
    }
        ...
        foreach (var precept in preceptsListForReading)
        {
            ...
            foreach (var pComp in comps)
            {
                if (pComp is PreceptComp_SelfTookMemoryThought preceptComp &&
                    (preceptComp.eventDef == EnhancedIdeologyDefOf.EB_DestroyedReligiousBook || preceptComp.eventDef == EnhancedIdeologyDefOf.EB_BookDestroyed))
                {
    ...
    private void SendDestructionLetter()
    {
        Find.LetterStack.ReceiveLetter(
            "EnhancedIdeology.LetterReligiousBookDestroyedLabel".Translate(),
            "EnhancedIdeology.LetterReligiousBookDestroyedText".Translate(parent.Named("BOOK"), Ideo.Named("IDEO")),
    ...
    private void ApplyImpactToFollowers(Map previousMap)
    {
        ...
        foreach (var pawn in pawns)
        {
            ...
            var thought = (Thought_ReligiousBookDestroyed)ThoughtMaker.MakeThought(EnhancedIdeologyDefOf.EB_ReligiousBookDestroyed);
    ...
    private Command_Action CreateBurnGizmo()
    {
        return new Command_Action
        {
            ...
            defaultDesc = Ideo != null
                ? "EnhancedIdeology.DestroyBookByBurningItUpsetIdeo".Translate(parent.Named("BOOK"), Ideo.Named("IDEO"))
                : "EnhancedIdeology.DestroyBookByBurningIt".Translate(parent.Named("BOOK")),
    ...
    private static void UpdateGizmoDisabled(Command_Action gizmo, List<Pawn> burners, Thing thing)
    {
        ...
        {
            gizmo.Disabled = true;
            gizmo.disabledReason = "EnhancedIdeology.NoColonistCanDestroy".Translate(thing.Named("BOOK"));
        }
        else
        {
    ...
    private List<FloatMenuOption> BuildBurnMenuOptions(List<Pawn> burners)
    {
        ...
        foreach (var pawn in burners)
        {
            ...
            {
                var text = Ideo != null
                    ? "EnhancedIdeology.ConfirmDestroyBookByBurningItUpsetIdeo".Translate(pawn.Named("PAWN"), parent.Named("BOOK"), Ideo.Named("IDEO"))
                    : "EnhancedIdeology.ConfirmDestroyBookByBurningIt".Translate(pawn.Named("PAWN"), parent.Named("BOOK"));

                Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(text, delegate ()
                {
                    var job = JobMaker.MakeJob(EnhancedIdeologyDefOf.EB_PlaceAndBurnUntilDestroyed, parent);
                    job.count = 1;
